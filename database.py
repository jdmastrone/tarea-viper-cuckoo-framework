...
class MalwareVT(Base):
    __tablename__ = 'malwarevt'

    id = Column(Integer(),primary_key=True)
    scan_id = Column(Text(),nullable=True)
    permalink = Column(Text(),nullable=True)
    resource = Column(Text(),nullable=True)
    verbose_msg = Column(Text(),nullable=True)
    scan_date = Column(DateTime(timezone=False),nullable=True)
    ...
    def __repr__(self):
        return "<MalwareVT ('{0}','{1}')>".format(self.id, self.scan_id)
...
class MalwareVTSc(Base):
    ...
    update = Column(Text(),nullable=True)
    ...
    def __repr__(self):
        return "<Malware_Scans ('{0}','{1}')>".format(self.id, self.antivirus)
...
class Database:
    ...
    def list_malwarevt(self):
        session = self.Session()
        rows = session.query(MalwareVT).all()
        return rows

    def add_malwarevt(self, sha256, scan_id, permalink, resource, verbose_msg, scan_date, positives, total):
        session = self.Session()
        ...
        try:
            new_malwarevt = MalwareVT(scan_id, permalink, resource, verbose_msg, scan_date, positives, total)
    ...
    def update_malware_task(self, id, task_id, json_data):
        session = self.Session()
        if sys.version_info < (3, 0):
            # on Python2 make sure to only handle ASCII
            try:
        ...
        try:
            session.query(Malware).get(id).task_id = task_id
    ...
    def update_malwarevt(self, resource, scan_date, positives, total):
        ...
        malwarevt_id = session.query(MalwareVT).filter(MalwareVT.resource == resource)
        try:
    ...
    def list_malwarevtsc(self):
        session = self.Session()
        rows = session.query(MalwareVTSc).all()
        return rows

    def add_malwarevtsc(self, sha256, antivirus, detected, version, result, update):
        session = self.Session()
        ...
        try:
            new_malwarevtsc = MalwareVTSc(antivirus, detected, version, result, update)
    ...
    def get_malwarevtsc(self, malwarevtsc_id):
        session = self.Session()
        malwarevtsc = session.query(MalwareVTSc).get(malwarevtsc_id)
    ...
    def update_malwarevtsc(self, sha256, antivirus, detected, version, result, update):
        session = self.Session()
        ...
        try:
            contador = 0
            for dato in session.query(MalwareVTSc).filter(MalwareVTSc.antivirus == antivirus).\
                                                   filter(MalwareVTSc.version == version).\
                                                   filter(MalwareVTSc.result == result):
                contador = contador + 1
